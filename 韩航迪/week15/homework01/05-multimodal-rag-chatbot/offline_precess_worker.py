...
def main():
    for msg in consumer:
        try:
            ...
            # 步骤2，使用mineru解析文件
            print(f"mineru -p {file_path} -o ./processed -b vlm-http-client -u http://127.0.0.1:30000")
            subprocess.check_output(f"mineru -p {file_path} -o ./processed -b vlm-http-client -u http://127.0.0.1:30000",
                                    shell=True, timeout=600)

            # 步骤3，使用bge和clip编码
            print(os.path.join("./precessed", os.path.basename(file_path).split(".")[0]) + "/**/**.md")
...
if __name__ == "__main__":
    ...
    # file_name = "LLM-Reasoning-Stanford-CS-25.pdf"
    # markdown_file_paths = glob.glob(os.path.join("./precessed", file_name.split(".")[0]) + "/**/**.md")
    # encode_document(markdown_file_paths[0], -1, file_name, file_name)